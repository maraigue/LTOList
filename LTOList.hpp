...
public:
    ...
    /// \~english
    /// The integer type that represents the difference of two indices in
    /// LTOList.
    ///
    /// \~japanese
    ...
    /// \~english
    /// The type of a constant-reference for elements that the LTOList can
    /// store.
    ///
    /// \~japanese
    ...
    struct Node {
        ...
        void recompute_n_children(size_type side)
        {
            ...
            if (child[side]) {
                n_children[side] =
                  1 + child[side]->n_children[0] + child[side]->n_children[1];
            } else {
                n_children[side] = 0;
            }
        }
        ...
        // this method does not return the result that `ch` is not a child of
        // `this`.
        //
        // NOTE 2:
        ...
        size_type side_of_child(pNode ch) { return (child[1] == ch ? 1 : 0); }
    };
    /// \endcond
    ...
    struct Base {
        ...
        // origin->parent (= origin->o_front()):  leftmost (smallest index) node
        // in the tree origin->child[0] (= origin->o_back()): rightmost (largest
        // index) node in the tree (To let `origin` work also as a sentinel, the
        // root node must be represented as a child of `origin`.)
        //
        // The setups above are determined for the purpose of the following
        // convenience:
        // 1. Either child of `origin` must be the root node, and
        //    `origin->side_of_child(root)` must correctly identify the side of
        //    `root` from `origin`. This is because of the convenience that the
        //    root node can have a (virtual) parent node.
        // 2. The previous element of `origin` computed by `neighbor`, namely
        // `neighbor(origin, 0)`,
        //    must be the last element that truly stores an element (i.e., other
        //    than `origin`).
        //
        // `origin->parent`, `origin->child[0]` and `origin->child[1]` must be
        // either "all of them are `nullptr`" or "none of them are `nullptr`".
        // The list is empty if and only if all of them are `nullptr`.
        pNode origin;

        Base()
          : origin(new Node(true))
        {
        }
        ~Base() { delete origin; }
        ...
        {
            return origin->o_root() ? (origin->o_root()->n_children[0] +
        ...
        static void display_tree_main(STREAM& sr,
                                      const pNode n,
                                      size_type depth,
                                      size_type offset)
        {
            ...
            sr << '<' << (n->red ? 'R' : 'B') << ">["
               ...
               << " parent=" << n->parent << " children=" << n->n_children[0]
               << "," << n->n_children[1] << "] " << n->value << std::endl;

            display_tree_main(sr, n->child[1], depth + 1,
        ...
        inline void display_tree(STREAM& sr) const
        {
            sr << "{LTOList Size=" << size() << " Origin=" << origin
               << " Root=" << origin->o_root() << " Front=" << origin->o_front()
        ...
        pNode add_main(pNode pos, size_type side, pNode newnode)
        {
            ...
            // Node addition cannot be conducted if either of the followings is
            // satisfied:
            ...
            // Since a node insertion must begin with a leaf to conduct
            // rotations, in case `pos->child[side]` is not `nullptr`, 1) Find
            // the `side` neighbor of `pos`, denoted by `pos_ins`. 2) Insert the
            // new node to the opposite of `side`.
            pNode pos_ins;
            size_type side_ins;
            if (pos->child[side] == nullptr) {
                ...
                // Since `pos->child[side]` is not `nullptr`, `neighbor(pos,
                // side)` cannot be `nullptr`
                side_ins = 1 - side;
            }
            ...
            while (rotpos[1]->red) {
                ...
                if (rotsides[0] == rotsides[1]) {
                    // (The figure below is rotsides[0] == 0; invert if
                    // rotsides[0] == 1)
                    //       [3]              [3]
                    //        :                :
                    //      b[2]             r[1]
                    ...
                    if (rotpos[2]->child[rotsides[0]])
                        rotpos[2]->child[rotsides[0]]->parent = rotpos[2];
                    // Connection 1-2
                    ...
                    if (rotpos[0]->child[rotsides[1]])
                        rotpos[0]->child[rotsides[1]]->parent = rotpos[1];
                    // Connection 2-B
                    rotpos[2]->child[rotsides[1]] =
                      rotpos[0]->child[rotsides[0]];
                    if (rotpos[0]->child[rotsides[0]])
                        rotpos[0]->child[rotsides[0]]->parent = rotpos[2];
                    // Connection 0-1
        ...
        pNode remove(pNode pos)
        {
            ...
            // - neighbor_pos: the neighboring element if the removed element
            // has two children, or null otherwise
            //
            // Noticing that
            // - returned_pos must be always identified, and
            // - neighbor_pos must be identified only if the removed element
            // (`pos`) has two children, we process as follows.
            if (pos == origin->o_back()) {
                // In case we cannot take `returned_pos` directly, that is,
                // `pos` is the last element, `pos` CANNOT have two children.
            ...
            while (adjust_needed && pos_adjust != origin) {
                ...
                // Here, pos_adjust->child[side_adjust] has fewer 'black'-height
                // by one than others
                ...
                if (pos_sibling->red) {
                    ...
                    // 'black'-height by one, we rotate it for [r]-[a]-[C]
                    // (processed next).
                    ...
                    if (pos_sibling->child[side_adjust])
                        pos_sibling->child[side_adjust]->parent = pos_adjust;
                ...
                // (The figure below is side_adjust == 0; invert if side_adjust
                // == 1) [a]: pos_adjust [r]: pos_reduced (possibly null) [s]:
                // pos_sibling
                // "%": fewer 'black'-height
                //
                //        [a]
                ...
                if (pos_sibling->child[side_adjust] &&
                    ...
                    if (pos_nephew->child[side_adjust])
                        pos_nephew->child[side_adjust]->parent = pos_adjust;
                    pos_sibling->child[side_adjust] =
                ...
                } else if (pos_sibling->child[1 - side_adjust] &&
                    ...
                    // be fewer by one than others, that is, we check the
                    // rotation once more.
    ...
    private:
        ...
        friend difference_type operator-(iterator it, iterator other)
        {
            assert(it.pbase_ == other.pbase_);
            return static_cast<difference_type>(it.pbase_->position(it.n_)) -
    ...
    private:
        // Global functions that accesses private members of `iterator`
        friend const_iterator operator+(const_iterator it, difference_type dir)
        {
            const_iterator after = it;
            after += dir;
            return after;
        }
        friend const_iterator operator+(difference_type dir, const_iterator it)
        {
            const_iterator after = it;
            after += dir;
            return after;
        ...
        friend difference_type operator-(const_iterator it,
        ...
        friend bool operator<(const_iterator it, const_iterator other)
        {
            assert(it.pbase_ == other.pbase_);
            return it.pbase_->position(it.n_) <
                   other.pbase_->position(other.n_);
        ...
        friend difference_type operator-(const_iterator it, iterator other)
        {
            assert(it.pbase_ == other.pbase_);
            return static_cast<difference_type>(it.pbase_->position(it.n_)) -
                   static_cast<difference_type>(
        ...
        friend difference_type operator-(iterator it, const_iterator other)
        {
            assert(it.pbase_ == other.pbase_);
            return static_cast<difference_type>(it.pbase_->position(it.n_)) -
                   static_cast<difference_type>(
...
public:
    ...
    iterator end()
    {
        return iterator(base_,
                        base_.origin->o_front() ? base_.origin : nullptr);
    }
    ...
    reference at(size_type i)
    {
        ...
        difference_type adpos =
          ...
          static_cast<difference_type>(base_.origin->o_root()->n_children[0]);
    ...
    /// \~english
    /// Inserts an element to this LTOList, before or after the specified
    /// element.
    ///
    /// @param side If it is 0, the new element is stored before `position`.
    /// Otherwise it is stored after `position`.
    ...
    /// の前に追加されます。そうでなければ `position` の後ろに追加されます。
    ...
    /// Otherwise it is stored after `position`.
    /// @param position The iterator at which the new element is stored.
    /// @param val The new element. It is "moved" to be stored into LTOList
    /// (i.e., the value in `val` may be deleted).
    ///
    /// \~japanese
    /// LTOList に新しい要素を、指定の要素の前か後ろに追加します。
    ///
    ...
    /// Otherwise it is stored after `position`.
    /// @param position The iterator at which the new element is stored.
    /// @param args The new element. It considers args as the arguments of the
    /// constructor of LTOList::value_type, and the constructed element is
    /// inserted into LTOList.
    ///
    /// \~japanese
    /// LTOList に新しい要素を、指定の要素の前か後ろに追加します。
    ///
    ...
    /// のコンストラクタの引数とし、その要素が LTOList に追加されます。
    template <class... Args>
    ...
    /// \~english
    /// Inserts specified number of elements to this LTOList, before or after
    ...
    /// \~english
    /// Same as
    /// insert(size_type side, iterator position, size_type n, const_reference
    /// val) with `side = 0` \.
    ///
    /// \~japanese
    /// insert(size_type side, iterator position, size_type n, const_reference
    /// val) で `side = 0` とした場合に同じです。
    void insert(iterator position, size_type n, const_reference val)
    {
        for (size_type i = 0; i < n; ++i)
            insert(0, position, val);
    }
    ...
    /// \~english
    /// Inserts a bunch of elements to this LTOList, before or after the
    /// specified element.
    ///
    /// @param side If it is 0, the new element is stored before `position`.
    /// Otherwise it is stored after `position`.
    ...
    /// LTOList::value_type is accepted; not limited to LTOList::iterator) for
    /// the first element to be inserted.
    /// @param elem_end The iterator (any iterator that points
    /// LTOList::value_type is accepted; not limited to LTOList::iterator)
    /// before the last element to be inserted.
    ///
    /// \~japanese
    ...
    /// の前に追加されます。そうでなければ `position` の後ろに追加されます。
    /// @param position 新しい要素を追加する位置を示すイテレータです。
    /// @param elem_begin 追加する最初の要素を表すイテレータ（ LTOList::iterator
    /// に限らず、 LTOList::value_type 型を指すイテレータなら何でもよい）です。
    /// @param elem_end 追加する最後の要素の手前（ LTOList::iterator に限らず、
    /// LTOList::value_type
    /// 型を指すイテレータなら何でもよい）を表すイテレータです。
    template <class InputIterator>
    void insert(size_type side,
                iterator position,
                ...
                InputIterator elem_end)
    {
        ...
        for (InputIterator it = elem_begin; it != elem_end; ++it) {
            ...
            side = 1;  // insertion side should be considered only for the first
                       // insertion
        }
    }
    ...
    /// \~english
    /// Same as
    /// insert(size_type side, iterator position, InputIterator elem_begin,
    /// InputIterator elem_end) with `side = 0` \.
    ///
    /// \~japanese
    /// insert(size_type side, iterator position, InputIterator elem_begin,
    /// InputIterator elem_end) で `side = 0` とした場合に同じです。
    template <class InputIterator>
    void
    insert(iterator position, InputIterator elem_begin, InputIterator elem_end)
    {
...
std::ostream& operator<<(std::ostream& os,
                         const typename LTOList<T>::iterator& it)
{
...
template <class T>
std::ostream& operator<<(std::ostream& os,
                         const typename LTOList<T>::const_iterator& it)
{
