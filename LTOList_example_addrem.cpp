...
void test_base(){
	...
	for(size_t num_orig_elems = 2; num_orig_elems <= 16; ++num_orig_elems){
		for(size_t id_rem_elem = 0; id_rem_elem < num_orig_elems; ++id_rem_elem){
			...
			std::cout << "num_orig_elems " << num_orig_elems << " id_rem_elem " << id_rem_elem << " returned " << (ot.remove(pos_removed) == pos_next) << std::endl;
		}
	}
}
...
void test_api(){
	...
	for(size_t num_orig_elems = 2; num_orig_elems <= 16; ++num_orig_elems){
		for(size_t id_rem_elem = 0; id_rem_elem < num_orig_elems; ++id_rem_elem){
			...
			for(size_t i = 0; i < num_orig_elems; ++i){
				LTOList<double>::iterator pos = ot.insert(ot.end(), i);
				if(i == id_rem_elem){
					pos_removed = pos;
				}else if(i == id_rem_elem + 1){
					pos_next = pos;
			...
			std::cout << "num_orig_elems " << num_orig_elems << " id_rem_elem " << id_rem_elem << " returned " << (ot.erase(pos_removed) == pos_next) << std::endl;
		}
	}
}
